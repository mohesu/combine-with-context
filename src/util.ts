...
function removeCommentsJS(code: string): string {
  ...
  while (i < code.length) {
    ...
    if (inCommentSingle) { if (c === '\n') {inCommentSingle = false;} i++; continue; }
    if (inCommentMulti) {
      if (c === '*' && i + 1 < code.length && code[i + 1] === '/') { i += 2; inCommentMulti = false; continue; }
      i++; continue;
    }
    if (inString) {
      output += c;
      if (c === stringChar && !isEscaped(code, i)) { inString = false; }
      i++; continue;
    }
    if (inRegex) {
      output += c;
      if (c === '/' && !isEscaped(code, i)) { inRegex = false; }
      i++; continue;
    }

    if (c === '"' || c === "'" || c === '`') {
      inString = true; stringChar = c; output += c; i++; continue;
    }

    if (c === '/' && i + 1 < code.length) {
      const next = code[i + 1];
      if (next === '/') { inCommentSingle = true; i += 2; continue; }
      if (next === '*') { inCommentMulti = true; i += 2; continue; }
      // regex
      inRegex = true; output += c; i++; continue;
    }
...
export function minifyContent(content: string, lang: string): string {
  ...
  switch (lowerLang) {
    case 'javascript':
    case 'typescript':
      cleaned = removeCommentsJS(content); break;
    case 'python':
      cleaned = removeCommentsPython(content); break;
    case 'css':
      cleaned = removeCommentsCSS(content); break;
  }
  let lines = cleaned.split('\n');
  if (indentDependentLangs.has(lowerLang)) {
    lines = lines.map(line => line.replace(/\s+$/, '')); // rstrip
  } else {
